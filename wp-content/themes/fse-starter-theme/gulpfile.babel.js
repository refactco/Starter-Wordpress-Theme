...
gulp.task('blocksStyles', () => {
	return gulp
		.src(config.blocksSRC, {allowEmpty: true, base: "./"})
		.pipe(plumber(errorHandler))
		.pipe(sass.sync({outputStyle: 'compressed'}).on('error', sass.logError))
		.pipe(autoprefixer(config.BROWSERS_LIST))
		.pipe(lineec()) // Consistent Line Endings for non UNIX systems.
...
gulp.task(
	'default',
	gulp.parallel('styles', 'blocksStyles', 'customJS', 'images', 'translate', browsersync, () => {
		gulp.watch(config.watchPhp, gulp.parallel('translate', reload)); // Reload on PHP file changes.
		gulp.watch(config.watchStyles, gulp.parallel('styles')); // Reload on SCSS file changes.
		gulp.watch(config.watchBlocksStyles, gulp.parallel('blocksStyles')); // Reload on Blocks SCSS file changes.
